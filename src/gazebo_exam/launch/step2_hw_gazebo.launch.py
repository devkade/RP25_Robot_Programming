...
def spawn_entity(context):
    ...
    models = {
        ...
        "box5": "box_red.xacro",
        "box6": "box_red.xacro",
        "cylinder": "cylinder.xacro",
        "triangle": "triangle.xacro",
    }
    models_entity ={
        "box1": {
            "xacro_args": {
        ...
        "cylinder": {
            "xacro_args": {
                ...
                "cylinder_offset_x": "0.5",
                "cylinder_offset_y": "0.0",
                "cylinder_offset_z": "0.03",
            }
        },
        "triangle": {
            "xacro_args": {
                "triangle_offset_x": "0.6",
                "triangle_offset_y": "0.1",
                "triangle_offset_z": "0.03",
            }
        },
    }
    ...
    xacro_path = os.path.join(package.perform(context), 'models', "case.xacro")
    # robot_description = xacro.process_file(xacro_path).toxml()
    robot_description_content = Command(
        [
            "xacro ",
            PathJoinSubstitution([package,"models","case.xacro"]),
        ]
    )
    ...
    robot_state_publisher_node = Node(
        package="robot_state_publisher",
        executable="robot_state_publisher",
        name="case_rsp", # 노드 이름 중복 방지
        output="screen",
        parameters=[{"use_sim_time": True},
                    {"robot_description": robot_description_content}],
        # 이 부분이 핵심입니다.
        remappings=[("/robot_description", "/case/robot_description")],
    )

    spawn_entity_node = Node(
        package="gazebo_ros",
        executable="spawn_entity.py",
        name="spawn_case", # 노드 이름 중복 방지
        arguments=[
            "-entity", "case",
            "-topic", "/case/robot_description",
            "-x", "0.0",
            "-y", "0.2",
            "-z", "0.0",],
        output="screen",
