                      ...
                      rclcpp::Node::SharedPtr node)
{
  ...
  start_pose = list_to_pose(0.55, 0.2, 0.5, -M_PI/2, 0, 0);
                   ...
                   rclcpp::Node::SharedPtr node)
{
  ...
  for (int i = 1; i <= num_waypoints; i++) {
    ...
    waypoint.position.y = center_y;
    waypoint.position.z = center_z + radius * sin(theta);

    waypoints.push_back(waypoint);
  }

  RCLCPP_INFO(logger, "Generated %zu waypoints for circle", waypoints.size());
