...
int main(int argc, char **argv)
{
  ...
  std::shared_ptr<rclcpp::Node> node = rclcpp::Node::make_shared("service_exam_client");
  rclcpp::Client<service_exam::srv::MyService>::SharedPtr client =
    node->create_client<service_exam::srv::MyService>("my_service");

  auto request = std::make_shared<service_exam::srv::MyService::Request>();
